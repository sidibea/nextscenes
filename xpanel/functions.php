...
function loginUser($username, $password){
	...
	if(mysql_num_rows($loginr) == 0){
		...
		$login_v = mysql_fetch_array($loginr);
...
function isLoggedIn(){
	if (!isset($_SESSION['ev_u_auth']) || $_SESSION['ev_u_auth'] == NULL ||  $_SESSION['ev_u_auth'] == "" ||  $_SESSION['ev_u_name'] == NULL){
...
function searchAllItems($type,$keyword, $num,$sort,$pager,$group,$tags,$statt){
	...
	if($type== "forum"){
		$itemq= "SELECT * FROM c_forums WHERE f_name LIKE \"%$searchstring%\"";
		/*$itemq .= "
		LEFT JOIN (
		...
		WHERE u_mod != 0
		AND (u_username LIKE ";
		$itemq .= "\"%$searchstring%\" OR (u_fname LIKE \"%$searchstring%\") OR (u_lastname LIKE \"%$searchstring%\")";
		$itemq .= ")";
	}
	else if($type== "user"){
		$itemq= "SELECT * FROM c_users
	...
	if ($name ==""){
		...
		if ($insr){
			if($upload['upload']["name"] != ""){
				$filename = uploadImage($upload);
				createThumbnail($filename);
				$insi = mysql_query("UPDATE c_storylines SET sl_img =\"$filename\",	sl_form = \"".uploadFrom()."\" WHERE sl_id =\"$cat\"");
				//echo $filename;
			}
...
function updateForum($name, $message, $parent, $upload, $cat){
	...
	if ($name ==""){
		...
		$message = mysql_real_escape_string($message);
		$parent = strip_tags(mysql_real_escape_string($parent));
		$cat = strip_tags(mysql_real_escape_string($cat));
		
		$insq = "UPDATE c_forums SET f_name =\"$name\",	f_desc =\"$message\" WHERE f_id = \"$cat\"";
		$insr = mysql_query($insq);
		if ($insr){
			if($upload['upload']["name"] != ""){
				$filename = uploadImage($upload);
				createThumbnail($filename);
				$insi = "UPDATE c_forums SET f_img = \"$filename\",	f_form = \"".uploadFrom()."\" WHERE f_id =\"$cat\"";
				$insir = mysql_query($insi);
			}
		...
		if ($value == "" || $value == NULL){
			...
			if($subtype == "proposed"){
				...
				if(mysql_num_rows($chkr) == 0){
					...
					$chk = mysql_fetch_array($chkr);
					$chkqx = "SELECT * FROM c_valid_scenes
							WHERE vs_scene =\"".$chk['ps_scene']."\" && c_storylines_sl_id = \"".$chk['c_storylines_sl_id']."\"";
					$chkrx = mysql_query($chkqx, $db_auth_evolve);
					$chkcount = mysql_num_rows($chkrx);
					$chkx = mysql_fetch_array($chkrx);
					$insq = "INSERT INTO c_valid_scenes (vs_desc, vs_scene, c_storylines_sl_id, c_users_u_id)
							VALUES (\"".mysql_real_escape_string($chk['ps_desc'])."\", \"".$chk['ps_scene']."\", \"".$chk['c_storylines_sl_id']."\", \"".$chk['c_users_u_id']."\")";
					$insr = mysql_query($insq);
					$scene_id = mysql_insert_id();
					if($insr){
						if($chkcount > 0){
							$insqx = "INSERT INTO c_proposed_scenes (ps_id, ps_desc, ps_scene, c_storylines_sl_id, c_users_u_id)
									VALUES (\"".$chkx['ps_id']."\", \"".mysql_real_escape_string($chkx['vs_desc'])."\", \"".$chk['vs_scene']."\", \"".$chk['c_storylines_sl_id']."\", \"".$chk['c_users_u_id']."\")";
						...
						$delr = mysql_query($delq, $db_auth_evolve);
						header("location: view_scene?scene=".$scene_id."&type=valid&alert=success");
						exit;
					}
				}
			}
...
function uploadImage($img){
	...
	$ar = explode('.', $img['upload']['name']);
	$fileExt = end($ar);
	echo $fileExt;
	
	$filename = md5(date('Ymd').time().'-').".".$fileExt;
	$target = imageDirectory().$filename;
	if(move_uploaded_file($img["upload"]['tmp_name'], $target)){
		return $filename;
	}else{
	...
	if ($type == "forum"){
		...
		if (mysql_num_rows($result)==0){
			...
			if($answer['f_form'] == "" || $answer['f_form'] == NULL){
				$data = "http://www.nextscenes.com/pictures/".stripslashes($answer['f_img']);
			}
			else{
				$data = "http://www.nextscenes.com/pictures/".stripslashes($answer['f_img']);
			}
...
function createStoryline($name, $mssg, $forum,$upload, $owner){
	...
	if ($name ==""){
		...
		$name = htmlspecialchars(mysql_real_escape_string(strip_tags($name)));
		$mssg = mysql_real_escape_string($mssg);
		
		//check if category name exists
		$chkq = "SELECT * FROM c_storylines WHERE sl_name =\"$name\"";
		$chkr = mysql_query($chkq);
		...
		if (mysql_num_rows($chkr)== 0){
			...
			$insq = "INSERT INTO c_storylines (sl_name, sl_desc, c_forums_f_id, c_users_u_id)VALUES (\"$name\", \"$mssg\",\"$forum\",\"$ownerid\")";
			$insr = mysql_query($insq);
			$forumID = mysql_insert_id($db_auth_evolve);
			if ($insr){
					$insqx = "INSERT INTO c_valid_scenes (vs_desc,vs_scene,c_storylines_sl_id,c_users_u_id)
							VALUES (\"$mssg\", '1', \"$forumID\", \"$ownerid\")";
					$insrx = mysql_query($insqx);
				if($upload['upload']["name"] != ""){
					$id = mysql_insert_id();
					$filename = uploadImage($upload);
					createThumbnail($filename);
					$insi = "UPDATE c_storylines SET sl_img = \"$filename\", sl_form =\"".uploadFrom()."\" WHERE sl_id =\"$forumID\"";
					$insir = mysql_query($insi);
				}
...
function getRatingForScene($scene){
	$i = 0;
	$query = mysql_query("SELECT * FROM unit1_rating WHERE pscene=\"$scene\"");
	if($query){
...
function pagingCustom($pages, $page, $per_page, $dlink, $tbname){
	$url = $dlink."/".$pages."?page";
	$query = "SELECT COUNT(*) as `num` FROM {$tbname}";
	$row = mysql_fetch_array(mysql_query($query));
	$total = $row['num'];
...
function personalApprove($id){
	$query = mysql_query("UPDATE c_topics SET publish='2' WHERE id=\"$id\"");
	if($query){
		$_SESSION['s'] = true;
...
function personalDecline($id){
	$query = mysql_query("UPDATE c_topics SET publish='3' WHERE id=\"$id\"");
	if($query){
		$_SESSION['d'] = true;
		header("location: pending");
		exit;
...
function personalRestore($id){
	$query = mysql_query("UPDATE c_topics SET publish='1' WHERE id=\"$id\"");
	if($query){
		$_SESSION['r'] = true;
		header("location: pending");
		exit;
...
function getSinglePostID($id){
	$query = mysql_query("SELECT * FROM c_topics WHERE id=\"$id\"");
	return mysql_fetch_array($query);
}
function adminEditPersonal($category, $content, $id){
	$query = mysql_query("UPDATE c_topics SET cat=\"$category\", content=\"$content\" WHERE id=\"$id\"");
	if($query){
		$_SESSION['s'] = true;
		...
		$_SESSION['f'] = true;
		header("location: edit-personal-story?id=$id");
		exit;
	}
}
function adminDeletePersonal($id){
	$query = mysql_query("DELETE FROM c_topics WHERE id=\"$id\"");
	if($query){
		$_SESSION['s'] = true;
		header("location: personal");
		exit;
...
function makePost($title, $content){
	...
	$cu = explode('_',$_SESSION['idsession']);
	$author = $cu[0];
	$sql = getSinglePost($links);
	$id = $sql['id'];
	if($id == ""){
		$sql = "INSERT INTO articles(topic, content, date, slug, author)VALUES(\"$title\", \"$content\", \"$date\", \"$links\", \"$author\")";
		$query = mysql_query($sql); echo mysql_error();
		if($query){
			$sql1 = getSinglePost($links);
			$_SESSION['s'] = true;
			$id = $sql1['id'];
			header("location: edit-article?id=$id");
...
function getSinglePost($links){
	$sql = "SELECT * FROM articles WHERE slug=\"$links\"";
...
function getSamplePost($id){
	$query = mysql_query("SELECT * FROM articles WHERE id=\"$id\"");
	return mysql_fetch_array($query);
}
function editPost($title, $content, $id){
	$query = mysql_query("UPDATE articles SET topic=\"$title\", content=\"$content\" WHERE id=\"$id\"");
	if($query){
		$_SESSION['s'] = true;
		header("location: articles");
		exit;
...
function getStory($id){
	global $db_auth;
	$query = "SELECT * FROM c_storylines WHERE sl_id =\"$id\"";
	$result = mysql_query($query);
...
function editScene($scene, $type, $content){
$content = mysql_real_escape_string($content);
	if($type == "proposed"){
		$query = mysql_query("UPDATE c_proposed_scenes SET ps_desc=\"$content\" WHERE ps_id=\"$scene\"");
		if($query){
			header("location: scenes");
			exit;
		}else{
			header("location: edit_scene.php?scene=$scene&type=$type");
			exit;
		}
	}elseif($type == "valid"){
		$query = mysql_query("UPDATE c_valid_scenes SET vs_desc=\"$content\" WHERE vs_id=\"$scene\"");
		if($query){
			header("location: scenes");
			exit;
		}else{
...
function countStories(){
	$itemq = mysql_query("SELECT * FROM c_storylines
		LEFT JOIN (
			SELECT COUNT(vs_id) AS validated_scenes, c_storylines_sl_id FROM c_valid_scenes 
			LEFT JOIN c_storylines ON c_storylines.sl_id = c_valid_scenes.c_storylines_sl_id 
			GROUP BY c_storylines_sl_id
		...
		LEFT JOIN c_users ON c_users.u_id = c_storylines.c_users_u_id");
	return mysql_num_rows($itemq);
}
function countForums(){
	$itemq = mysql_query("SELECT * FROM c_forums
		LEFT JOIN (
			SELECT COUNT(sl_id) AS story_count, c_forums_f_id FROM c_storylines
			GROUP BY c_forums_f_id
		) ff ON ff.c_forums_f_id = c_forums.f_id
...
function countValidScenes(){
	...
	$itemq .= " AND scene_id IN (SELECT vs_id FROM c_valid_scenes)";
	return mysql_num_rows(mysql_query($itemq));
}
function countProposedScenes(){
	$itemq = "SELECT * FROM (
			SELECT vs_id AS scene_id, vs_ts AS scene_ts, vs_desc AS scene_desc, vs_scene AS scene_number, '0'  AS scene_status, c_users_u_id AS scene_user_id, c_storylines_sl_id AS scene_storyline, 'valid' AS scene_type FROM c_valid_scenes
			UNION 
			SELECT ps_id, ps_ts, ps_desc, ps_scene, ps_status, c_users_u_id, c_storylines_sl_id, 'proposed' AS scene_type FROM c_proposed_scenes
...
function countUsers(){
	$itemq = mysql_query("SELECT * FROM c_users
		LEFT JOIN (
			SELECT COUNT(ra_id) AS rating_count, c_users_u_id FROM c_ratings
			GROUP BY c_users_u_id
		) cc ON cc.c_users_u_id = c_users.u_id
...
function countPersonalStories(){
	$query = mysql_query("SELECT * FROM c_topics");
	return mysql_num_rows($query);
}
function findThem($number){
	$query = mysql_query("SELECT * FROM c_users ORDER BY u_id DESC LIMIT 0, $number");
	while($row = mysql_fetch_array($query)){
		$data[] = $row;
	}
	return $data;
...
function deleteUser($id){
	$query = mysql_query("DELETE FROM c_users WHERE u_id=\"$id\"");
	if($query){
...
function getPage($id){
	$query = mysql_query("SELECT * FROM data WHERE Iddata=\"$id\"");
	return mysql_fetch_array($query);
}
function editPaged($id, $content){
	$content = mysql_real_escape_string($content);
	$query = mysql_query("UPDATE data SET Descriptions=\"$content\" WHERE Iddata=\"$id\"");
	if($query){
		header("location: ./");
		exit;
	}else{
