...
<title>Super User || Pending Personal Stories</title>
<?php include("include/head.php");?>
<h1>Pending Personal Stories</h1>
...
<?php if($_SESSION['r'] == true){echo "<div class=\"alert alert-success\">Story Restored For Review.</div>";}$_SESSION['r'] = false;?>
<?php
	...
	$limit =20;
	$tlink = $slug;
	$tbname = "c_topics WHERE publish=1 ORDER BY id DESC";
	$url = "";
	...
	<div style="padding:10px; border-radius:10px; border:1px solid #EFEFEF">
		...
		<div class="space"></div>
		<div style="font-size:12px;"><a href="../story-<?php echo $row['slug'];?>" target="_blank">View</a> | <?php if($row['mode'] == 0){echo "Public";}?><?php if($row['mode'] == 1){echo "Private";}?><?php if($row['mode'] == 3){echo "Group Story";}?> | By: <?php echo $row['name'];?> | <a href="edit-personal-story?id=<?php echo $row['id'];?>">Edit</a> | <a href="mint?action=approve&id=<?php echo $row['id'];?>">Approve</a> | <a href="mint?action=decline&id=<?php echo $row['id'];?>">Decline</a> | <a href="mint?action=delete-personal-story&id=<?php echo $row['id'];?>" onclick="return confirm('Are you sure you want to DELETE this story?')"><span style="font-weight:bold; color:red;">Delete</span></a></div>
	</div>
	<?php }?>
