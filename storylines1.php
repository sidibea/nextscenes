...
<title>Nextscenes.com - Forum <?php echo $forumName['f_name']; ?></title>
		...
		<div class="row">
			...
			<div class="row">					
            	...
            	<?php
					foreach ($story as $story_o){
						...
						if(empty($_SESSION['idsession'])){
							$the_url = "<a style=\"display:none;\" href=\"login1\" class=\"clickthis multi\">&rarr; ";
							...
							if($story_o['storylines']=='0'){ 
								...
								if(!isset($_SESSION['language']) || ($_SESSION['language'] == "en")){
									...
									$the_url .= "Histoire Inactive";
								}
								$the_url .= '</a> ';
							}
							else {
								 $the_url = '<a href="/'.custom_site_base().'read1-'.urlThis($story_o['sl_name']).'-'.$story_o['sl_id'].'" class="clickthis multi">&rarr; ';
								if(!isset($_SESSION['language']) || ($_SESSION['language'] == "en")){
									$the_url .= "View scenes";
								}
								else{
				...
				<div class=\"c_post\">
					...
					<div class=\"c_view\">
						<a href=\"".custom_site_base()."read1-".urlThis($story_o['sl_name'])."-".$story_o['sl_id']."\"><h3 class=\"h3\">".utf8_encode(stripslashes(htmlspecialchars_decode($story_o['sl_name'])))."</h3></a>
						...
						<div class=\"clear\"></div>";
							...
							echo "<div>".Encoding::fixUTF8(shorten($story_o['sl_desc'],180))."</div>".$the_url."</div></div>";
						}
					?></div>
					</div>
				</div>
