...
<body>
	...
	<div class="inner-banner">
		<div class="note"><?php echo $data0['Forum'] ;  ?> Story Lines (<?php 
									$query = 'SELECT COUNT(*) FROM storylines WHERE IdForum="'.mysql_clean($id).'"';
...
$sql4 = 'SELECT * FROM scenes_valides WHERE idstory = "'.mysql_clean($data1['idstory']).'" ORDER BY scenes DESC LIMIT 0,1';
...
$query = 'SELECT COUNT(*) FROM scenes_proposes WHERE idstory = "'.mysql_clean($data1['idstory']).'" AND scenes="'.$data4['scenes'].'"';
$total = mysql_fetch_array(mysql_query($query));
...
<div class="blog-box">';
 ...
 if(empty($data1['Filename']))  { echo '<img src="/pictures/avatar.jpg" alt="">'; } else { echo ' <img src="/image.php?width=275&height=300&cropratio=3:2&image=/pictures/'.$data1['Filename'].'" alt=""></a>'; }
				...
				<div class="blog-text-mscenes">
					...
					<div class="post-text"> '.ucfirst(html_entity_decode(coupechaine($data4['Text'], 200))).'</div>
                     <div class="mscenes"><ul>
					<li><a href="scenes-'.$data4['scenes'].'-'.reecUrl($data1["Title"]).'-'.$data1['idstory'].'.html">Scènes Proposées</a></li>
					

';
                     
                     if ($dataUser['Account'] == 'Power') { echo '<li><a href="scenesvalidate-'.reecUrl($data1["Title"]).'-'.$data1['idstory'].'.html">Scènes Validées</a></li> ' ; } 
