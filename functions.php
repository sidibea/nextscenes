...
function getPosts($the_page){
	...
	global $db_auth;
	$query = "(SELECT ps_id AS the_main_id, ps_ts, ps_desc, ps_scene, ps_status, c_proposed_scenes.c_users_u_id, c_storylines_sl_id, f_id, f_name, u_avatar, u_username, sl_id, sl_name, sl_views FROM c_proposed_scenes
...
(SELECT vs_id AS the_main_id, vs_ts, vs_desc, vs_scene, '1' AS vs_status, c_valid_scenes.c_users_u_id, c_storylines_sl_id, f_id, f_name, u_avatar, u_username, sl_id, sl_name, sl_views FROM c_valid_scenes
	...
	<div class=\"main\">
		...
		<ul>";
			...
			$data .= "
				...
				<li><a href=\"/".custom_site_base()."register\">New member? Sign up</a></li>
...
<div class=\"w100p\">
	<div class=\"main\">
    	<ul class=\"nav\">
        	<span>
                <li><a href=\"/".custom_site_base()."\"><img src=\"images/logo.png\" alt=\"\" class=\"c_logo\"></a><div class=\"mobile_menu mobile_menu_open hide\"></div><div class=\"mobile_menu mobile_menu_close hide\"></div></li>
            </span>
        	...
        	<span class=\"mobile_menu_option\">
                <li><a href=\"/".custom_site_base()."login\""; if($page == "login"){ $data .= " class=\"selected\""; } $data .= ">";
				if(!isset($_SESSION['language_session']) || ($_SESSION['language_session'] == "en")){
					$data .= "Log in";
				}
				else{
				...
				$data .= "</a></li>
                <li><a href=\"/".custom_site_base()."register\""; if($page == "signup"){ $data .= " class=\"selected\""; } $data .= ">";
				if(!isset($_SESSION['language_session']) || ($_SESSION['language_session'] == "en")){
					$data .= "Sign up";
				}
				else{
...
<div class=\"tright\">
	...
	if($new != 0){
		...
		foreach ($new as $new_o){
			$imgstylex = getImgStyle("avatars/".$new_o['u_avatar']);
			if(!empty($new_o['avatar'])){
				$avatar = "<img src=\"avatars/".$new_o['u_avatar']."\"/>";
			}
			else{
...
function getForumStories($id, $page){
	...
	$query = "SELECT * FROM c_storylines
		...
		LEFT JOIN (
			...
			) iddd ON iddd.c_storylines_sl_id = storylines.sl_id
		LEFT JOIN (
			SELECT COUNT(ps_id) AS proposed_scenes_count, c_storylines_sl_id FROM c_proposed_scenes
			GROUP BY c_storylines_sl_id
			) idx ON idx.c_storylines_sl_id = storylines.sl_id
		LEFT JOIN c_users ON c_users.u_username = storylines.c_usrs_u_id
