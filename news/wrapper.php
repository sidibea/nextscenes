<?php
	...
	$sq1 = "SELECT * FROM cat WHERE slug=\"$tlink\"";
	$q1 = $db->query($sq1);
...
<title>NextScenes &bull; <?php if($db->num_rows($q1)>0){echo $db->name($rows);}elseif($db->num_rows($q2)>0){echo $db->topic($rows);}else{echo "Page Not Found";}?></title>
<meta charset="utf-8">
<!--[if IE]>
...
<div class="herald-section container">
	...
	$limit =20;
	$tbname = "topics WHERE cat=\"$cat\" ORDER BY id DESC";
		...
		<!-- ARTICLE STARTS -->
            <article class="col-lg-6 col-md-6">
                <div class="picture">
                    <div class="category-image">
                        <a href="<?php echo $db->dlink()."/".$db->slug($row);?>"><img src="<?php echo $db->firstImage($db->content($row));?>" class="img-responsive" alt="<?php echo $db->topic($row)?>" /></a>
			...
			?>
					...
					<div class="description">
						...
						$db->query("UPDATE topics SET views=views+1 WHERE slug=\"$tlink\"");
						?>
					</div>
					<div class="detail">
