...
@$forums = mysql_escape_string($_POST['forums'] ) ;
@$texte = mysql_escape_string($_POST['FCKeditor1']) ;


if(!empty($titre) && !empty($texte) && !empty($forums) )
{
...
if(!empty($fichier))
{
	...
	$sql="UPDATE $t2 SET Title='$titre', Forums='$forums', Description='$texte', filename='$fichier' WHERE IdScenes='$op1'";
	...
	echo'<script language="javascript">
        ...
        {
			$sql="UPDATE $t2 SET Title='$titre', Forums='$forums', Description='$texte' WHERE IdScenes='$op1'";
	mysql_query($sql) or die('Erreur SQL !'.$sql.'<br />'.mysql_error());
	redirect_to('manage.php');
