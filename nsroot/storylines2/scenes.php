...
$sql1 = 'SELECT id,Date,Login,Text,valide, (totalrate / nrrates) AS rank FROM scenes_proposes WHERE idstory = "'.mysql_clean($id).'" AND scenes="'.mysql_clean($sc).'" ORDER BY rank DESC LIMIT 0,1'; // ( totalrate / nrrates) 
...
';
if ( empty($data1['valide']) ){ echo '<a href=valide.php?op='.$data1["id"].'> <strong><h5>>>Validate this scenes</h5></strong> </a>'; } else { echo' <strong><h5>This scenes is Validated</h5></strong> '; }
