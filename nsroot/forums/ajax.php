...
include'../../includes/connect_db.php';
		...
		$fields = array("IdForum" => "Id", "DateCreate" => "Date", "Title" => "Title" ); // , "Scenes" => "Scenes", 
...
<tbody>
	<?php
		@$sql = "SELECT count(*) AS num FROM $table $search_sql $order_by LIMIT $pageLimit"; //  where rub='news'
		...
		@$sql = "SELECT $select_fields FROM $table $search_sql $order_by LIMIT $start, $pageLimit"; // where rub='news' 
		$row = mysql_query ( $sql );
...
*/
             echo "<td align=left width=100> $result[IdForum] </td>"; // <a href=\"stocks_insert.php?op=$result[IdProduit]\">
			...
			// echo "<td align=left> <img src=../../pictures/$result[Filename] width=25 height=25> </td>";
			  echo "<td align=left><a href=\"modif.php?op1=$result[IdForum]\">$result[Title]</a></td>"; // ".CoupeChaine($result[titre], 50)." <a href=\"stocks_insert.php?op=$result[IdProduit]\"> </a>
			//  echo "<td align=left> $result[lien] </td>";  echo "<td align=left width=130> $result[HeureUNE] </td>"; //  </a>
			 // echo "<td align=left width=120> $result[Scenes] </td>"; 
			   echo "<td align=left><a href=\"supp.php?op=$result[IdForum]\"><img src=../image/supp.png border=0 width=16 height=16></a></td>"; 
...
<tfoot>
	<tr><td colspan="30"><h2><?php echo $rowCount . " Forum(s) created "; ?></h2></td></tr>
</tfoot>
</table>
</div>
