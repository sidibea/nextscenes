...
class Rating {
    ...
    if(!empty($rate)) {
      ...
      if($this->rater === '') return "alert('Vote Not registered.\\nYou must be logged in to can rate')";
        ...
        if(in_array($items[0], $rtgstdy)) {
          $rtgstdy[] = $items[0];
          setcookie("ratings", implode(',', array_unique($rtgstdy)), strtotime('tomorrow+1200 month'));
  ...
  protected function setRtgDb($items, $rate, $rtgstdy) {
	...
	// $this->sqlExecute("INSERT INTO `scenes_proposes` (`totalrate`) VALUES ('1')");
	 $this->sqlExecute("UPDATE `scenes_proposes` SET `totalrate`=`totalrate`+$rate, `nrrates`=`nrrates`+1 WHERE id='".$items[0]."'");
	 
	// $this->sqlExecute("INSERT INTO `$this->rtgvote` (`item`, `vote`) VALUES ('".$items[0]."', $rate')");
	  $this->sqlExecute("INSERT INTO rtgvote (`item`, `vote`) VALUES ('".$items[0]."', '".$rate."') ");
  ...
  protected function setRtgTxt($items, $rate, $all_rtgstdy) {
    ...
    setcookie("ratings", implode(',', array_unique($all_rtgstdy[$this->tdy])), strtotime('tomorrow+1200 month'));
  }
