...
class MySQLiDatabase{
	...
	public function dlink2(){
		return "http://www.nextscenes.com/fr/news";
	}
	...
	public function getSinglePost($links){
		$sql = "SELECT * FROM nfr_topics WHERE slug=\"$links\"";
	...
	public function getPostByID($id){
		$sql = "SELECT * FROM nfr_topics WHERE id=\"$id\"";
		$query = $this->query($sql);
		$row = $this->fetch_array($query);
		return $row;
	}
	public function makePost($title, $content, $cat){
		...
		$id = $sql['id'];
		if($id == ""){
			$sql = "INSERT INTO nfr_topics(topic, content, date, slug, author, cat)VALUES(\"$title\", \"$content\", \"$date\", \"$links\", \"$author\", \"$cat\")";
			$query = $this->query($sql);
			if($query){
	...
	public function updatePost($id, $title, $content){
		...
		if($id != ""){
			$sql = "UPDATE nfr_topics SET topic=\"$title\", content=\"$content\" WHERE id=\"$id\"";
			$query = $this->query($sql);
			if($query){
				$_SESSION['s'] = true;
				header("location: xpanel/edit?id=$id");
	...
	public function catName($id){
		$sql = "SELECT * FROM nfr_cat WHERE id=\"$id\"";
		$query = $this->query($sql);
	...
	public function getAuthor($id){
		$sql = "SELECT * FROM nfr_membuser WHERE id=\"$id\"";
		$query = $this->query($sql);
		if($this->num_rows($query)>0){
	...
	public function deletePost($id){
		$sql = "DELETE FROM nfr_topics WHERE id=\"$id\"";
		$query = $this->query($sql);
		if($query){
			$_SESSION['s'] = true;
	...
	public function getLogin($username, $password){
		return "SELECT * FROM nfr_membuser WHERE username=\"$username\" && password=\"$password\"";
	}
	public function findUser($username){
		return "SELECT * FROM nfr_membuser WHERE username=\"$username\"";
	}
	public function findUserEmail($email){
		return "SELECT * FROM nfr_membuser WHERE email=\"$email\"";
	}
	public function findUserID($email){
		return "SELECT * FROM nfr_membuser WHERE id=\"$email\"";
	}
	...
	public function newUser($username, $email, $name, $password, $role){
		...
		if($this->num_rows($query1) == 0){
			...
			if($this->num_rows($q1)>0){
				...
				//Insert Into DB
				$sql = "INSERT INTO nfr_membuser(username, email, name, password, role)VALUES(\"$username\", \"$email\", \"$name\", \"$password\", \"$role\")";
				$query = $this->query($sql);
				if($query){
					$_SESSION['s'] = true;
	...
	public function editUser($username, $email, $name, $password, $id){
		...
		if(empty($password)){
			$sql = "UPDATE nfr_membuser SET username=\"$username\", name=\"$name\", email=\"$email\" WHERE id=\"$id\"";
		}else{
			$password = sha1($password);
			$sql = "UPDATE nfr_membuser SET username=\"$username\", name=\"$name\", email=\"$email\", password=\"$password\" WHERE id=\"$id\"";
		}
		$query = $this->query($sql);
		if($query){
			$_SESSION['s'] = true;
	...
	public function deleteUser($id){
		$sql = "DELETE FROM nfr_membuser WHERE id=\"$id\"";
		$query = $this->query($sql);
		if($query){
			$_SESSION['s'] = true;
			header("Location: xpanel/users");
	...
	public function editProfile($name, $password){
		...
		if(empty($names) || empty($id)){
			$_SESSION['required'] = true;
			header("Location: xpanel/edit-profile");
			exit;
		}
		if(empty($password)){
			$sql = "UPDATE nfr_membuser SET name=\"$name\" WHERE id=\"$id\"";
		}else{
			$password = sha1($password);
			$sql = "UPDATE nfr_membuser SET name=\"$name\", password=\"$password\" WHERE id=\"$id\"";
		}
		$query = $this->query($sql);
		if($query){
			$_SESSION['s'] = true;
	...
	public function allCatOption(){
		$query = $this->query("SELECT * FROM nfr_cat");
		if($this->num_rows($query)>0){
	...
	public function catByID($id){
		$query = $this->query("SELECT * FROM nfr_cat WHERE id=\"$id\"");
		$row = $this->fetch_array($query);
		return $row;
	}
	public function likePost($str){
		...
		if(isset($_COOKIE['link']) && $_COOKIE['link'] == $str){
			$query = $this->query("SELECT love FROM nfr_topics WHERE slug=\"$str\"");
			$row = $this->fetch_array($query);
			$rate = $this->love($row);
			//Voted
			$data = 2;
			...
			//Please add vote
			$this->query("UPDATE nfr_topics SET love = love+1 WHERE slug=\"$str\"");
				$query = $this->query("SELECT love FROM nfr_topics WHERE slug=\"$str\"");
				$row = $this->fetch_array($query);
				$rate = $this->love($row);
			$data = 1;
	...
	public function dislikePost($str){
		...
		if(isset($_COOKIE['link']) && $_COOKIE['link'] == $str){
			...
			//Please add vote
				$this->query("UPDATE nfr_topics SET dislove = dislove+1 WHERE slug=\"$str\"");
				$query = $this->query("SELECT dislove FROM nfr_topics WHERE slug=\"$str\"");
				$row = $this->fetch_array($query);
	...
	public function lovePost($str){
		...
		$data = 0;
		if(isset($_COOKIE['link']) && $_COOKIE['link'] == $str){
			$query = $this->query("SELECT love FROM nfr_articles WHERE slug=\"$str\"");
			$row = $this->fetch_array($query);
			$rate = $this->love($row);
			//Voted
			$data = 2;
			...
			//Please add vote
			$this->query("UPDATE nfr_articles SET love = love+1 WHERE slug=\"$str\"");
				$query = $this->query("SELECT love FROM nfr_articles WHERE slug=\"$str\"");
				$row = $this->fetch_array($query);
				$rate = $this->love($row);
			$data = 1;
	...
	public function dislovePost($str){
		...
		if(isset($_COOKIE['link']) && $_COOKIE['link'] == $str){
			...
			//Please add vote
				$this->query("UPDATE nfr_articles SET dislove = dislove+1 WHERE slug=\"$str\"");
				$query = $this->query("SELECT dislove FROM nfr_articles WHERE slug=\"$str\"");
				$row = $this->fetch_array($query);
				$rate = $this->dislove($row);
	...
	public function totalPosts($id){
		$query = $this->query("SELECT * FROM nfr_topics WHERE author=\"$id\"");
	...
	public function setTraffic(){
		date_default_timezone_set("Africa/Lagos");
		$date = date('Y-m-d');
		$s0 = $this->query("SELECT * FROM nfr_traffic");
		$s1 = $this->query("SELECT * FROM nfr_traffic WHERE date=\"$date\"");
		if($this->num_rows($s1)>0){
			$this->query("UPDATE nfr_traffic SET count=count+1 WHERE date=\"$date\"");
		}else{
			$this->query("INSERT INTO nfr_traffic(date, count)VALUES(\"$date\", \"1\")");
		}
...
//		$geo = var_export(unserialize(file_get_contents("http://www.geoplugin.net/php.gp?ip=".$user_ip)));
		...
		$sq1 = $this->query("SELECT * FROM nfr_country WHERE name=\"$country\" && date=\"$date\"");
		if($this->num_rows($sq1)>0){
			$this->query("UPDATE nfr_country SET hits=hits+1 WHERE name=\"$country\" && date=\"$date\"");
		}else{
			$this->query("INSERT INTO nfr_country(name, hits, date)VALUES(\"$country\", \"1\", \"$date\")");
		}
		$ref = parse_url($_SERVER['HTTP_REFERER'], PHP_URL_HOST);
		if($ref == ""){
			$ref = "Annonymous";
		}
		$ref1 = $this->query("SELECT * FROM nfr_ref WHERE name=\"$ref\" && date=\"$date\"");
		if($this->num_rows($ref1) > 0){
			$this->query("UPDATE nfr_ref SET hits=hits+1 WHERE name=\"$ref\" && date=\"$date\"");
		}else{
			$this->query("INSERT INTO nfr_ref(name, hits, date)VALUES(\"$ref\", \"1\", \"$date\")");
		}
	}
    public function viewTraffic(){
		$datas = '';
        $date = date('Y-m-d', strtotime("today"));
        $sq2 = $this->query("SELECT * FROM nfr_country WHERE date=\"$date\" ORDER BY hits DESC");
        $ref2 = $this->query("SELECT * FROM nfr_ref WHERE date=\"$date\" ORDER BY hits DESC");
        //Get data for today
        $query = $this->query("SELECT * FROM nfr_traffic WHERE date=\"$date\"");
        $rs = $this->fetch_array($query);
        $date = date('Y-m-d', strtotime("today"));
            //Get data for yesterday
            $dd = date('Y-m-d', strtotime("-1 day"));
            $query1 = $this->query("SELECT * FROM nfr_traffic WHERE date=\"$dd\" ORDER BY oid");
            $ry = $this->fetch_array($query1);
            //Get data for Last Seven days
            $ddw = date('Y-m-d', strtotime("-1 week"));
            $sev = 0;
            $query2 = $this->query("SELECT * FROM nfr_traffic WHERE date ='".$ddw."'");
            if($query2){
            ...
            $mon = 0;
            $query3 = $this->query("SELECT * FROM nfr_traffic WHERE date='".$ddx."' ORDER BY oid DESC");
            ...
            $yer = 0;
            $query4 = $this->query("SELECT * FROM nfr_traffic WHERE date='".$ddz."' ORDER BY oid DESC");
            while($rz = $this->fetch_array($query4)){
                $yer += $rz['count'];
            }
        //Get data for All Time
            $query5 = $this->query("SELECT * FROM nfr_traffic ORDER BY oid DESC");
            $rp = $this->fetch_array($query5);
            $query6 = $this->query("SELECT * FROM nfr_traffic ORDER BY oid ASC");
            $rp6 = $this->fetch_array($query6);
            $dbx = $rp['date'];
            $dbx6 = $rp6['date'];
            $all = 0;
            $query5 = $this->query("SELECT * FROM nfr_traffic WHERE date BETWEEN '".$dbx6."' AND '".$dbx."' ORDER BY oid DESC");
		...
		public function newCat($name){
			$slug = $this->fuckpbnl($name);
			$query = $this->query("INSERT INTO nfr_cat(name, slug)VALUES(\"$name\", \"$slug\")");
			if($query){
				$_SESSION['s'] = true;
				header("location: xpanel/category");
				exit;
			}else{
				$_SESSION['f'] = true;
				header("location: xpanel/category");
