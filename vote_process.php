<?php
	...
	$db_username = 'nextscendidb';
	$db_password = 'Sidere852';
	$db_name = 'nextscendidb';
	$db_host = 'og21315-001.privatesql:35357';
...
if($_POST)
{
	...
	$result0 = mysqli_query($sql_con,"SELECT idstory FROM nscenes WHERE id='$unique_content_id'");
	$get_total_rows0 = mysqli_fetch_assoc($result0);
	$idstory = $get_total_rows0['IdScene'];
	...
	{			
		...
		case 'up': 
			...
			if($get_total_rows)
			{
				...
				mysqli_query($sql_con,"UPDATE voting_count SET vote_up=vote_up+1, idstory=$idstory  WHERE unique_content_id='$unique_content_id'");
			}else{
				//no record found, insert new record in db
				mysqli_query($sql_con,"INSERT INTO voting_count (idstory, unique_content_id, vote_up) value('$idstory','$unique_content_id',1)");
				//mysqli_query($sql_con,"INSERT INTO voting_count (unique_content_id, vote_up, IdScene) value('$unique_content_id',1,'')");
			}
		...
		case 'down': 
			...
			if($get_total_rows)
			{
				...
				mysqli_query($sql_con,"UPDATE voting_count SET vote_down=vote_down+1, idstory=$idstory WHERE unique_content_id='$unique_content_id'");
			}else{
				
				//no record found, insert new record in db
				mysqli_query($sql_con,"INSERT INTO voting_count (idstory, unique_content_id, vote_down) value('$idstory', '$unique_content_id',1)");
			}
